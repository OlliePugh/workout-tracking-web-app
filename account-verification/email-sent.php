<!DOCTYPE html>
<html lang="en">
  <head>
    <title>Flex-Fit</title>
    <link rel="stylesheet" type="text/css" href="../css/style.css">
    <link rel="stylesheet" type="text/css" href="../css/login.css"> <!--Has very similair setup to the login page -->
  </head>
  <body>
    <div style="text-align: center; position: relative; top: 50%; transform: translateY(-50%);">
      <div id="wrapper">
        <a href="../index.php"><img src="../css/images/logo-with-text.png" style="height:100px; margin-bottom: 10px;" alt="Flex-Fit logo"></a>
        <h1 style="color: white; margin: 5px 0;">Account Authentication Email Sent</h1>
        <p>The email may take 5-10 minutes to send. Please check your spam box if you do not find it in your inbox.</p>
        <a href="resend-email.php?email=<?php echo($_GET["email"]);?>" style="color: var(--accent-color)">Send email again</a>
      </div>
    </div>
  </body>
</html>
